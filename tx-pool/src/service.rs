...
impl TxPoolServiceBuilder {
    pub fn new(
        ...
        txs_verify_cache: Arc<Mutex<TxVerifyCache>>,
        ...
        let server = move |handle: Handle| {
            let process = receiver.for_each(move |message| {
                ...
                handle.spawn(async move {
                    service_clone.process(message).await;
                });
                future::ready(())
            });
...
pub struct TxPoolService {
    ...
    pub(crate) txs_verify_cache: Arc<Mutex<TxVerifyCache>>,
    ...
    async fn process(&self, message: Message) {
        match message {
            Message::GetTxPoolInfo(Request { responder, .. }) => {
                let info = self.tx_pool.lock().await.info();
            ...
            Message::BlockTemplate(Request {
                ...
                let block_template_result = self
            ...
            Message::SubmitTxs(Request {
                ...
                let submit_txs_result = self.process_txs(txs).await;
            ...
            Message::NewUncle(Notify { arguments: uncle }) => {
                if self.block_assembler.is_some() {
