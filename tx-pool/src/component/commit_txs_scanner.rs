...
impl<'a> CommitTxsScanner<'a> {
        ...
        self.proposed_pool.with_sorted_by_score_iter(|iter| {
            ...
            loop {
                ...
                Self::next_candidate_tx(
                    ...
                    cycles,
                    |entry| {
                        let tx_id = entry.rtx.transaction.proposal_short_id();
                ...
                debug_assert!(!self
                    .fetched_txs
                    .contains(&tx_entry.rtx.transaction.proposal_short_id()));
                // prepare to package tx with ancestors
                let mut ancestors = self
                    .proposed_pool
                    .get_ancestors(&tx_entry.rtx.transaction.proposal_short_id())
                    .into_iter()
                    .filter_map(|short_id| {
                ...
                for entry in ancestors {
                    let short_id = entry.rtx.transaction.proposal_short_id();
    ...
    fn update_modified_entries(&mut self, new_fetched_txs: &HashSet<TxEntry>) {
        for ptx in new_fetched_txs {
            let ptx_id = ptx.rtx.transaction.proposal_short_id();
