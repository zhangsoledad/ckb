...
use ckb_core::cell::{
    resolve_transaction, OverlayCellProvider, ResolvedTransaction, UnresolvableError,
};
...
impl TxPool {
    ...
    pub fn update_tx_pool_for_reorg<'a>(
        ...
        let txs_iter = attached.iter().map(|tx| {
            let get_cell_data =
                |out_point: &OutPoint| snapshot.get_cell_data(&out_point.tx_hash, out_point.index);
