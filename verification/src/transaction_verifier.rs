...
impl<'a> DuplicateDepsVerifier<'a> {
    ...
    pub fn verify(&self) -> Result<(), TransactionError> {
        ...
        if transaction.deps().into_iter().all(|id| seen.insert(id)) {
            Ok(())
        } else {
                ...
                input
                    ...
                    .map(|cell| {
                        cell.cell_output
                            ...
                            .to_opt()
                            .map(|type_| {
                                let code_hash: H256 = type_.code_hash().unpack();
        ...
        if since.is_relative() {
            ...
            match since.extract_metric() {
                ...
                Some(SinceMetric::Timestamp(timestamp)) => {
                    ...
                    let cell_median_timestamp = self.parent_median_time(&cell.hash.pack());
