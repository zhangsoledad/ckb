...
use ckb_error::Error;
use ckb_types::{
    core::{BlockExt, BlockView, EpochExt, HeaderView, TransactionInfo, cell::CellMeta},
    packed,
    prelude::*,
};
use std::sync::Arc;
...
impl StoreTransaction {
            ...
            if let Some(cellbase) = &block.transactions().first() {
                if let Some(output) = cellbase.outputs().get(0) {
                    if let Some(output_data) = cellbase.outputs_data().get(0) {
                        ...
                        let mut guard = cache.cell_meta.lock();

                        ckb_logger::info!(
                            "cell_meta insert {} cache size {}",
                            cellbase.hash(),
                            guard.len(),
                        );
                        guard.insert(cellbase.hash(), cell_meta);
                    }
                }
            }
        }
    ...
    pub fn remove_cellbase_meta_cache(&self, block: &BlockView) {
        if let Some(cache) = self.cache() {
            let header = block.header();
            if let Some(cellbase) = &block.transactions().first() {
                let mut guard = cache.cell_meta.lock();
                ckb_logger::info!(
                    "cell_meta remove size {}",
                    guard.len()
                );
                guard.remove(&cellbase.hash());
            }
        }
    }
