        ...
        block.transactions().iter().rev().for_each(|tx| {
            ...
            if !tx.is_cellbase() {
                tx.inputs().into_iter().for_each(|input| {
                    if let Some(cell_out_point) = input.previous_output().cell().to_opt() {
...
impl IndexerStoreTransaction {
    ...
    fn delete_cell_out_point_lock_hash(&self, cell_out_point: &CellOutPoint) {
        self.txn
            .delete(COLUMN_CELL_OUT_POINT_LOCK_HASH, cell_out_point.as_slice())
    ...
    fn get_lock_hash_cell_output(
        ...
        self.txn
            .get(COLUMN_CELL_OUT_POINT_LOCK_HASH, cell_out_point.as_slice())
            .expect("indexer db read should be ok")
            .map(|value| {
