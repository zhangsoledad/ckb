...
impl<'a, CS: ChainStore<'a>> DaoCalculator<'a, CS, DataLoaderWrapper<'a, CS>> {
    ...
    pub fn maximum_withdraw(
        ...
        let cell_out_point = out_point.cell().to_opt().ok_or(Error::InvalidOutPoint)?;
                    ...
                    let capacity: Result<Capacity, FailureError> = match &resolved_input.cell() {
                        ...
                        Some(cell_meta) => {
                            ...
                            if output
                                ...
                                .to_opt()
                                .map(|t| {
                                    let code_hash: H256 = t.code_hash().unpack();
                                ...
                                let withdraw_header_hash = rtx
                                    ...
                                    .and_then(|dep_index| {
                                        rtx.transaction
                                            ...
                                            .and_then(|out_point| out_point.block_hash().to_opt())
                                            .ok_or(Error::InvalidOutPoint)
                                    })?;
                                self.calculate_maximum_withdraw(
                                    &output,
...
mod tests {
    ...
    use ckb_types::{
        core::{
            capacity_bytes,
            cell::{CellMetaBuilder, ResolvedOutPoint},
            BlockBuilder, BlockNumber, HeaderBuilder, TransactionBuilder,
        },
        h256, H256, U256,
    };
    ...
    fn check_withdraw_calculation() {
        ...
        let out_point = OutPoint::new(deposit_block.hash().unpack(), tx.hash().unpack(), 0);
