...
impl<'a, CS: ChainStore<'a>> DaoCalculator<'a, CS, DataLoaderWrapper<'a, CS>> {
    ...
    pub fn dao_field<'b, R: Iterator<Item = &'b ResolvedTransaction> + Clone>(
        &self,
        rtxs: R,
        parent: &HeaderView,
    ) -> Result<Byte32, Error> {
    ...
    fn added_occupied_capacities<'b, R: Iterator<Item = &'b ResolvedTransaction> + Clone>(
        &self,
        mut rtxs: R,
    ) -> Result<Capacity, Error> {
    ...
    fn withdrawed_interests<'b, R: Iterator<Item = &'b ResolvedTransaction> + Clone>(
        &self,
        mut rtxs: R,
    ) -> Result<Capacity, Error> {
...
mod tests {
    ...
    fn check_dao_data_calculation_overflows() {
        ...
        let (store, parent_header) = prepare_store(&parent_header, None);
        let result = DaoCalculator::new(&consensus, &store).dao_field([].iter(), &parent_header);
        assert!(result
            .unwrap_err()
            .to_string()
