...
impl DatabaseMigration {
    ...
    pub fn open<P: AsRef<Path>>(path: P) -> Result<Option<Self>, Error> {
        ...
        let db = RocksDB::prepare_for_bulk_load_open(path, COLUMNS)?;

        Ok(db.map(|db| DatabaseMigration { db, migrations }))
    }
...
impl Launcher {
    ...
    pub fn migrate_guard(&self) -> Result<(), ExitCode> {
        if let Some(migration) =
            DatabaseMigration::open(&self.args.config.db.path).map_err(|err| {
                eprintln!("{}", err);
                ExitCode::Failure
            })?
        {
