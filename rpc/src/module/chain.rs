...
impl ChainRpc for ChainRpcImpl {
    ...
    fn get_current_epoch(&self) -> Result<EpochView> {
        Ok(self
            ...
            .map(|ext| EpochView::from_ext(self.shared.consensus().epoch_reward(), ext.pack()))
    ...
    fn get_epoch_by_number(&self, number: EpochNumber) -> Result<Option<EpochView>> {
        Ok(self
            ...
            .and_then(|hash| {
                self.shared.store().get_epoch_ext(&hash).map(|ext| {
                    EpochView::from_ext(self.shared.consensus().epoch_reward(), ext.pack())
                })
            }))
    }
                    ...
                    for (i, output) in transaction.outputs().into_iter().enumerate() {
                            ...
                            && transaction_meta.is_dead(i) == Some(false)
                        {
                            ...
                            let out_point = packed::OutPoint::new_builder()
                                .cell(Some(cell).pack())
                                .block_hash(Pack::pack(&Some(block_hash.clone())))
                                .build();
